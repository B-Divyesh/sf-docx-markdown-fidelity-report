...
- Browser entry: `https://docx-markdown-fidelity-report.sociobot.in/demo`
...
- Browser behavior: plays a recording produced from the same bundled sample. It stores no document data. Demo state uses only the `demo:` local-storage prefix; **Reset demo** clears that prefix. **Start for real** leaves demo mode and discards demo state.
